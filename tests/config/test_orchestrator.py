...
from mlcast.config import convgru_training_experiment, ldcast_training_experiment, train_from_config
...
@patch("mlcast.config.orchestrator.fdl.build")
def test_train_from_config_valid_ldcast(mock_build: Any, tmp_path: Path) -> None:
    """Verify that a valid LDCast configuration passes validation and builds."""
    mock_build.return_value.trainer.log_dir = str(tmp_path)
