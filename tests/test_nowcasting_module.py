...
from mlcast.nowcasting_module import NowcastLightningModule
...
def test_nowcasting_module_forward_uses_network_shape_contract() -> None:
    """NowcastLightningModule should call fixed-shape forecasting networks as network(x)."""
    network = DummyForecastNetwork(input_steps=3, forecast_steps=5, ensemble_size=2)
    module = NowcastLightningModule(network=network, loss_class="crps")
