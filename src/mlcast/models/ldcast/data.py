...
class AutoencoderDataset(Dataset):
    ...
    def __init__(self, sampled_radar_dataset, autoenc_time_ratio=4):
        ...
        self.autoenc_time_ratio = autoenc_time_ratio
        self.samples_ratio = int(
            self.srd.steps / self.autoenc_time_ratio
        )  # is 6 in the usual case where steps = 24 and autoenc_time_ratio = 4
    ...
    def __getitem__(self, idx):
        """
        When given idx between 0 and 6 * len(srd) - 1, first find which
        sample of srd we are in (index_srd). Then find which partition of
        that sample we are in (index_in_srd_sample).
        """
        index_srd = idx // self.samples_ratio
        index_in_srd_sample = idx - index_srd * self.samples_ratio
        x = self.srd[index_srd].reshape(self.samples_ratio, 1, self.autoenc_time_ratio, self.srd.w, self.srd.h)[
            index_in_srd_sample
        ]
...
class DataModule(pl.LightningDataModule):
    def __init__(
        self,
        dataset: LatentDataset | AutoencoderDataset,
        train_ratio=0.6,
        val_ratio=0.2,
        **dataloader_kwargs,
    ):
