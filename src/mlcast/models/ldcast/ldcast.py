...
from .data import AutoencoderDataset, DataModule, LatentDataset
...
class LDCast(NowcastingModelBase):
    ...
    def fit_ldm(
        ...
        # need to add a trainer instance in the LatentDataset class to
        # automatically move the autoencoder to cuda etc.
        dataloader_kwargs = dataloader_kwargs or {}
        trainer_kwargs = trainer_kwargs or {}
    ...
    def predict(self, inputs, num_diffusion_iters=50, verbose=True):
        ...
        raise AssertionError(
            "prediction should be implemented with a trainer to take into "
            "account the switches of ema weights, for example"
        )
        ...
        with contextlib.redirect_stdout(None):
            (s, intermediates) = self.sampler.sample(
                num_diffusion_iters, batch_size, gen_shape, condition, progbar=verbose
            )
